...
import { decrypt, comparePassword } from "./crypto";
  ...
  if (auth != null) {
    if (
      ...
      let [results] = await db.query(
        `SELECT id as userId, password as hash FROM todo.users WHERE username = ?`,
        [username],
      );
      if (results.length === 0) {
      ...
      let { userId, hash } = results[0];
      let passwordMatched = await comparePassword(password, hash.toString());
      if (!passwordMatched) {
        throw new AuthError(req, {
          status: 401,
          code: "invalid-credentials",
...
export async function checkPermissions(req, db, userId, listId, role) {
  let [results] = await db.query(
    `SELECT role FROM todo.permissions WHERE userId = ? AND listId = ?`,
    [userId, listId],
  );
